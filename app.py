...
# ------------------------------------------------------------------------------
# โฟลเดอร์ที่เก็บไฟล์โมเดล .pkcls (ให้ดาวน์โหลดไฟล์จาก Google Drive โฟลเดอร์ Model
# แล้ววางไว้ในโฟลเดอร์เดียวกับ app.py นี้ หรือใต้โฟลเดอร์ MODEL_DIR ด้านล่าง)
MODEL_DIR = "."
...
# ------------------------------------------------------------------------------
def find_model_files(folder: str = MODEL_DIR):
    """ค้นหาไฟล์ .pkcls ทั้งหมดในโฟลเดอร์ที่กำหนด"""
    pattern = os.path.join(folder, "*.pkcls")
...
available_models = find_model_files(MODEL_DIR)

uploaded_model = st.file_uploader(
    "หรืออัปโหลดไฟล์โมเดล .pkcls เอง (เช่น ดาวน์โหลดมาจาก Google Drive โฟลเดอร์ Model)",
    type=["pkcls"],
)

model_path = None
if uploaded_model is not None:
    # บันทึกไฟล์ที่อัปโหลดลงดิสก์ชั่วคราวเพื่อให้ joblib.load อ่านได้
    temp_path = os.path.join(".", uploaded_model.name)
    with open(temp_path, "wb") as f:
        f.write(uploaded_model.getbuffer())
    model_path = temp_path
elif available_models:
    model_choice = st.selectbox(
        "เลือกไฟล์โมเดล (.pkcls) ที่พบในโฟลเดอร์ของแอป",
        available_models,
    ...
    st.warning(
        "ไม่พบไฟล์ .pkcls ในโฟลเดอร์ของแอป กรุณาวางไฟล์ Logistic.pkcls / "
        "Neural.pkcls / Tree.pkcls (ดาวน์โหลดจากโฟลเดอร์ Model บน Google Drive) "
        "ไว้ในโฟลเดอร์เดียวกับ app.py หรืออัปโหลดไฟล์ด้านบนแทน"
    )
